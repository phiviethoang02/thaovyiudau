...
class Particle1 {
  ...
  render() {
    drawHeart(this.canvas, this.x, this.y, this.radius * 10, this.color);
  }
...
class microParticle {
  ...
  render() {
    drawHeart(this.canvas, this.x, this.y, this.radius *4, this.color);
  }
...
class TextParticle {
  ...
  render() {
    ...
    this.canvas.fillStyle = this.color;
    this.canvas.font = `bold ${this.fontSize}px Comic Sans MS, sans-serif`;
...
setInterval(() => {
  textParticles.push(new TextParticle(canvas));
}, 100);
...
function update() {
  ...
  textParticles = textParticles.filter(function(tp) {
    return tp.move();
  });
